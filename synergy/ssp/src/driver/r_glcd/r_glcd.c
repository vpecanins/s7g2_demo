...
static ssp_err_t r_glcd_open_param_check_sync_signal (display_cfg_t const * const p_cfg)
{
    ...
    /* See S7G2 H/W manual BG_HSIZE.HP about 5cycles */
    GLCD_ERROR_RETURN((p_cfg->output.htiming.back_porch >= 5), SSP_ERR_INVALID_ARGUMENT);

    /* See S7G2 H/W manual BG_VSIZE.VP about 2cycles */
    GLCD_ERROR_RETURN((p_cfg->output.vtiming.back_porch >= 2), SSP_ERR_INVALID_ARGUMENT);
 ...
 * @param[in]     p_cfg        Pointer to the configuration structure for display interface
 * @param[in]     p_glcd_reg   Pointerto GLCD registers
 * @retval        void
 **********************************************************************************************************************/
static void r_glcd_clock_set (R_GLCDC_Type * p_glcd_reg, display_cfg_t const * const p_cfg)
{
...
static void r_glcd_sync_signal_set (R_GLCDC_Type * p_glcd_reg, display_cfg_t const * const p_cfg)
{
    ...
    r_glcd_data_enable_set(p_glcd_reg, tcon_de, &p_cfg->output.vtiming, &p_cfg->output.htiming, p_cfg->output.data_enable_polarity);
                                      ...
                                      display_frame_layer_t const frame)
{
    ...
    r_glcd_graphics_plane_format_set(p_glcd_reg, p_input->format, frame);
...
static void r_glcd_gamma_correction (R_GLCDC_Type * p_glcd_reg, display_cfg_t const * const p_cfg)
{
    ...
    if (p_cfg->output.p_gamma_correction->g.enable)
    {
        ...
        {
            HW_GLCD_GammaCorrectionGreenGainUpper16BitsSet(p_glcd_reg, i, p_cfg->output.p_gamma_correction->g.gain[i * 2]);
            HW_GLCD_GammaCorrectionGreenGainLower16BitsSet(p_glcd_reg, i, p_cfg->output.p_gamma_correction->g.gain[(i * 2) + 1]);
        }
    ...
    if (p_cfg->output.p_gamma_correction->b.enable)
    {
        for (i = 0; i < (DISPLAY_GAMMA_CURVE_ELEMENT_NUM / 2); i++)
        {
            HW_GLCD_GammaCorrectionBlueGainUpper16BitsSet(p_glcd_reg, i, p_cfg->output.p_gamma_correction->b.gain[i * 2]);
            HW_GLCD_GammaCorrectionBlueGainLower16BitsSet(p_glcd_reg, i, p_cfg->output.p_gamma_correction->b.gain[(i * 2) + 1]);
        }
    ...
    if (p_cfg->output.p_gamma_correction->r.enable)
    {
        for (i = 0; i < (DISPLAY_GAMMA_CURVE_ELEMENT_NUM / 2); i++)
        {
            HW_GLCD_GammaCorrectionRedGainUpper16BitsSet(p_glcd_reg, i, p_cfg->output.p_gamma_correction->r.gain[i * 2]);
            HW_GLCD_GammaCorrectionRedGainLower16BitsSet(p_glcd_reg, i, p_cfg->output.p_gamma_correction->r.gain[(i * 2) + 1]);
        }
